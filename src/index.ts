...
import { z } from "zod";
import { listProjects, listWorkers, listTasklists, listTasks, createTask } from "./freelo.js";

const email = process.env.FREELO_EMAIL;
const apiKey = process.env.FREELO_API_KEY;

if (!email || !apiKey) {
  console.error("Missing FREELO_EMAIL or FREELO_API_KEY environment variables");
  process.exit(1);
}
...
server.tool(
  "list_projects",
  "List all Freelo projects with their IDs",
  {},
  async () => {
